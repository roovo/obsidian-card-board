...
boardZipper =
    describe "boardZipper"
        ...
        , test "returns empty boards as a SafeZipper if there are some boards configured but the TaskList is empty" <|
            \() ->
                Boards.init "b1" (SafeZipper.fromList [ BoardConfig.fromBoardType "dateBoard" "dateboard title" ]) TaskList.empty
                    |> Boards.boardZipper
                    |> Expect.equal (SafeZipper.fromList [ Board.init "b1" (BoardConfig.fromBoardType "dateBoard" "dateboard title") TaskList.empty ])
        , test "returns boards (containing all TaskItems) as a SafeZipper if there are boards configured and ther are tasks" <|
            \() ->
                Boards.init "b1" (SafeZipper.fromList [ BoardConfig.fromBoardType "dateBoard" "dateboard title" ]) TaskListHelpers.taskListFromFileG
                    |> Boards.boardZipper
                    |> Expect.equal (SafeZipper.fromList [ Board.init "b1" (BoardConfig.fromBoardType "dateBoard" "dateboard title") TaskListHelpers.taskListFromFileG ])
        ]
...
currentIndex =
    describe "currentIndex"
        ...
        , test "returns 0 if there are boards and the current index of the configs is 0" <|
            \() ->
                Boards.init "b1" (SafeZipper.fromList [ BoardConfig.fromBoardType "dateBoard" "dateboard title", BoardConfig.fromBoardType "dateBoard" "another dateboard title" ]) TaskList.empty
                    |> Boards.currentIndex
                    |> Expect.equal (Just 0)
        , test "returns 1 if there are boards and the current index of the configs is 1" <|
            \() ->
                Boards.init "b1" (SafeZipper.next <| SafeZipper.fromList [ BoardConfig.fromBoardType "dateBoard" "dateboard title", BoardConfig.fromBoardType "dateBoard" "another dateboard title" ]) TaskList.empty
                    |> Boards.currentIndex
                    |> Expect.equal (Just 1)
        ]
