...
fromElmTests =
    describe "interop.fromElm (encoding)"
        ...
        , test "encodes TrackDraggable data" <|
            \() ->
                { beaconIdentifier = "someBeaconId", clientPos = { x = 1.1, y = 2.2 }, draggableId = "id of draggable" }
                    |> InteropDefinitions.TrackDraggable
                    |> TsEncode.runExample interop.fromElm
                    |> .output
                    |> Expect.equal """{"tag":"trackDraggable","data":{"beaconIdentifier":"someBeaconId","clientPos":{"x":1.1,"y":2.2},"draggableId":"id of draggable"}}"""
...
toElmTests =
    describe "interop.toElm (decoding)"
        ...
        , test "decodes elementDragged data" <|
            \() ->
                """{"tag":"elementDragged","data":{"beaconIdentifier":"some-beacon-id","dragAction":"stop","cursor":{"x":1.23,"y":4.56},"offset":{"x":1.11,"y":2.22},"draggedNodeRect":{"x":1.1,"y":2.2,"width":3.33,"height":4.44},"beacons":[{"beaconPosition":{"identifier":"someId","position":"before"},"rect":{"x":1.1,"y":2.2,"width":3.3,"height":4.4}}]}}"""
                    |> DecodeHelpers.runDecoder interop.toElm
                    |> .decoded
                    |> Expect.equal
                        (Ok <|
